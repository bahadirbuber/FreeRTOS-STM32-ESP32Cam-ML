...
/* Definitions for jpgDecode */
osMessageQueueId_t jpgDecodeHandle;
const osMessageQueueAttr_t jpgDecode_attributes = {
  .name = "jpgDecode"
};
/* Definitions for afterJPGDecode */
osMessageQueueId_t afterJPGDecodeHandle;
const osMessageQueueAttr_t afterJPGDecode_attributes = {
  .name = "afterJPGDecode"
};
/* USER CODE BEGIN PV */
osThreadId_t jpegDecodeTaskHandle;
const osThreadAttr_t jpegDecodeTask_attributes = {
  .name = "jpegDecodeTask",
  .stack_size = 1028 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};

osMessageQueueId_t FPSQueueHandle;
const osMessageQueueAttr_t FPSQueue_attributes = {
  .name = "FPSQueue"
};
int totalLength = 0;
/* USER CODE END PV */
...
uint8_t* buffer8;
__IO uint8_t *ptrSDRAMaddr=(__IO uint8_t*)(RAMbufferJPG);

uint8_t semapNullval=1;
...
{   
  ...
  DMA2D_Handle.Init.OutputOffset = 480 - xsize; 
  ...
  destination = (uint32_t)pDst + ((y * 480) + x) * 4;
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  /* creation of afterJPGDecode */
  afterJPGDecodeHandle = osMessageQueueNew (2, sizeof(uint8_t), &afterJPGDecode_attributes);

  /* USER CODE BEGIN RTOS_QUEUES */
  /* add queues, ... */
  FPSQueueHandle = osMessageQueueNew (2, sizeof(uint8_t), &FPSQueue_attributes);
...
void StartJPEGDecodeTask(void *argument){
	...
	uint8_t fps = 0;
	TickType_t tickCount1=xTaskGetTickCount();
	TickType_t tickCount2=tickCount1;//xTaskGetTickCount();
	  const TickType_t countTICKfor1second= pdMS_TO_TICKS(1000);
	for(;;){
		if(osMessageQueueGet(jpgDecodeHandle, &totalLength_,NULL, 0U)==osOK){
			TickType_t tickCount1=xTaskGetTickCount();
			if(tickCount1 - tickCount2 >= countTICKfor1second){
				fps = frameCount;
				osMessageQueuePut(FPSQueueHandle, &fps,0, 0);
			...
			JPEG_Decode_DMA(&hjpeg, (uint32_t)RAMbufferJPG2, totalLength ,(uint32_t) RAMbufferJPG);
			while(Jpeg_HWDecodingEnd == 0)
			{
			}
			HAL_JPEG_GetInfo(&hjpeg, &JPEG_Info);
			DMA2D_CopyBuffer((uint32_t *)RAMbufferJPG, (uint32_t *)buffer8, 0 , 0, JPEG_Info.ImageWidth, JPEG_Info.ImageHeight, JPEG_Info.ChromaSubsampling);
...
//	uint32_t xPos = 0, yPos = 0;
	...
	for(;;)
	{
	  if(osMessageQueueGet(spiQueueHandle,m_Rx_Buffer , 0, 0) == osOK){
		  if (strncmp((char *)m_Rx_Buffer, "start", 5) == 0) {
			  offsetIndPacket = 0;
			  char *semicolonPtr = strchr((char *)m_Rx_Buffer, ';');
			  if (semicolonPtr != NULL) {
				  totalLength = atoi(semicolonPtr + 1); // Sayısal değeri çıkar
			  }
		  } else {
			   if(((offsetIndPacket+1)*BUFFER_SIZE) <= totalLength){
				   memcpy((uint8_t *)(RAMbufferJPG2+offsetIndPacket*BUFFER_SIZE),m_Rx_Buffer,BUFFER_SIZE);
				   offsetIndPacket++;
			   }else{
				   uint8_t val = 1;
				   osMessageQueuePut(jpgDecodeHandle, &val, 0U, 0U);
				   offsetIndPacket = 0;
			   }
...
void MPU_Config(void)
{
  ...
  */
  ...
  MPU_InitStruct.BaseAddress = 0x24000584;
